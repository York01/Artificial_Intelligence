...
def find_maximum(index, new_genes, genes, demand, capacity, cost_matrix):
    current_col = None
    max_cost = None
    length = len(genes)
    if index == -1:
...
def combine_solutions(genes, agreement_matrix, cost_matrix, depot, capacity):
    '''
    ...
    while new_length < length:
        ...
        next_col, _ = find_maximum(-1, new_genes, genes, demand, capacity, agreement_matrix)
...
def find_cost_matrix(genes):
    ...
    closest neighbor for WoAC when a node is not available
...
def solve_CVRP(parameters, cvrp_filename):
    ...
    for i in range(5, 25, 5):
        ...
        result_WoAC = combine_solutions(new_genes, agreement_matrix, cost_matrix, depot, capacity)
